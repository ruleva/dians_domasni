...
def analyze():
    try:
        filepath = request.form.get('filepath')  # Assume the user provides a filepath
        response = requests.post('http://localhost:5002/analyze', json={'filepath': filepath})
        if response.status_code == 200:
            flash("Analysis completed successfully!", "success")
        else:
            flash("Failed to analyze data. " + response.json().get("error", ""), "error")
    except Exception as e:
        flash(f"Error connecting to analysis service: {e}", "error")
    return redirect(url_for('welcome'))
